...
func SetupRoutes(app *fiber.App, db *store.PostgresStore, producer *queue.Producer, cfg *config.Config, gateway *realtime.Gateway, registry *provider.Registry) {
	...
	datasources.Post("/", RequireScope("datasource:write"), datasourceHandler.CreateDatasource)
	datasources.Get("/", RequireScope("datasource:read"), datasourceHandler.ListDatasources)
	datasources.Get("/:id", RequireScope("datasource:read"), datasourceHandler.GetDatasource)
	datasources.Patch("/:id", RequireScope("datasource:write"), datasourceHandler.UpdateDatasource)
	datasources.Delete("/:id", RequireScope("datasource:write"), datasourceHandler.DeleteDatasource)

	// Batch notifications
	batchHandler := NewBatchHandler(db, producer)
	batches := v1.Group("/batches")
	batches.Post("/send", RequireScope("batch:write"), batchHandler.SendBulk)
	...
	batches.Post("/:id/cancel", RequireScope("batch:write"), batchHandler.CancelBatch)
	batches.Post("/:id/retry", RequireScope("batch:write"), batchHandler.RetryBatch)
	batches.Delete("/:id", RequireScope("batch:write"), batchHandler.DeleteBatch)
