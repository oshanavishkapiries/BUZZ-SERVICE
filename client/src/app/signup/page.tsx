...
export default function SignupPage() {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (res.application && res.application.id) {
				localStorage.setItem('buzz_active_app_id', res.application.id);
			}
	...
	return (
		<div className="flex min-h-screen items-center justify-center p-4 bg-[var(--bg-primary)]">
			<div className="w-full max-w-md p-8 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-secondary)] shadow-xl relative overflow-hidden">
				...
				<div className="absolute -top-40 -left-40 w-80 h-80 bg-[var(--accent)] opacity-10 rounded-full blur-3xl pointer-events-none"></div>
				...
				<div className="flex flex-col items-center mb-8 relative z-10">
					...
					<p className="text-sm text-[var(--text-muted)] mt-1">Get started with Buzz multi-client notification service</p>
				</div>
